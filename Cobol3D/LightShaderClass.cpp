...
namespace Cobol {
	...
	bool LightShaderClass::Init(ID3D11Device * device, HWND hwnd) {
		bool result = InitShader(device, hwnd, "Light.hlsl", "Light.hlsl");
		return result;
	}
	void LightShaderClass::Shutdown() {
		ShutdownShader();
	}
	void LightShaderClass::Render(ID3D11DeviceContext * devCon, int indexCount,ID3D11ShaderResourceView * texture, 
		XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor,XMFLOAT4 ambientColor,XMFLOAT4 specularColor,float specularPower)
	{
		SetShaderParameters(devCon,texture,lightDirection,diffuseColor,ambientColor,specularColor,specularPower);

		RenderShader(devCon,indexCount);
	}
	bool LightShaderClass::InitShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename)
	{
		HRESULT result;
		...
		result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "LightVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
		if(FAILED(result))
		{
			if(errorMessage)
			{
		...
		result = D3DX11CompileFromFile(psFilename, NULL, NULL, "LightPS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
		if(FAILED(result))
		{
			if(errorMessage)
			{
	...
	void LightShaderClass::OutputShaderErrorMessage(ID3D10Blob* blob, HWND hwnd, CHAR* shaderFilename) {
	...
	bool LightShaderClass::SetShaderParameters(ID3D11DeviceContext * devCon,ID3D11ShaderResourceView * texture,
		XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor, XMFLOAT4 ambientColor,XMFLOAT4 specularColor,float specularPower) {
	...
	void LightShaderClass::RenderShader(ID3D11DeviceContext * devCon, int indexCount) {
