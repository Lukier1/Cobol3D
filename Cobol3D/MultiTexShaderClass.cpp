...
namespace Cobol {
	...
	bool MultiTexShaderClass::Init(ID3D11Device * device, HWND hwnd) {
		bool result = InitShader(device, hwnd, "MultiTex.hlsl", "MultiTex.hlsl");
		return result;
	}
	void MultiTexShaderClass::Shutdown() {
		ShutdownShader();
	}
	void MultiTexShaderClass::Render(ID3D11DeviceContext * devCon, int indexCount, ID3D11ShaderResourceView ** texture)
	{
		SetShaderParameters(devCon, texture);

		RenderShader(devCon, indexCount);
	}
	bool MultiTexShaderClass::InitShader(ID3D11Device* device, HWND hwnd, CHAR* vsFilename, CHAR* psFilename)
	{
		HRESULT result;
		...
		result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "MultiTexVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
		if (FAILED(result))
		{
			if (errorMessage)
			{
		...
		result = D3DX11CompileFromFileA(psFilename, NULL, NULL, "MultiTexPS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &pixelShaderBuffer, &errorMessage, NULL);
		if (FAILED(result))
		{
			if (errorMessage)
			{
	...
	void MultiTexShaderClass::OutputShaderErrorMessage(ID3D10Blob* blob, HWND hwnd, CHAR* shaderFilename) {
	...
	bool MultiTexShaderClass::SetShaderParameters(ID3D11DeviceContext * devCon, ID3D11ShaderResourceView ** textureArray) {
		devCon->PSSetShaderResources(0, 3, textureArray);
		return true;
	}
	void MultiTexShaderClass::RenderShader(ID3D11DeviceContext * devCon, int indexCount) {
